        ...
        <tbody>
            @forelse($items as $data)
                        ...
                        <div class="d-flex align-items-start gap-4 nowrap">
                            <div class="checkout-pro-img m-0 position-relative">
                                ...
                                <span data-id="{{$data->id}}"
                                    class="remove-product addtocart-remove-btn remove-cart-data"><i
                                        class="fa-solid fa-xmark"></i></span>
                            </div>
                            <div class="check-item">
                                <h4 class="product-title pb-1">
                                    <a
                                        href="{{route('product.details',[$data->product->slug ? $data->product->slug : make_slug($product->name),$data->product->id])}}">
                                        {{limit_words($data->product->name,2)}}
                                    </a>
                                </h4>
                                <p class="text-muted fs-12 lh-1">{{short_amount($data->price - $data->total_taxes)}} x
                ...
                @php
                    ...
                    $subTotal += ($data->original_price*$data->quantity);
            ...
            @empty
                <tr>
                    <td class="border-bottom-0" colspan="100">
                            @include('pos.partials.empty_cart')
                    </td>
                </tr>
            @endforelse
...
<ul>
    <li class="d-flex align-items-center justify-content-between gap-4 subtotal">
        <span class="fw-semibold  py-4  fs-14 nowrap">
    ...
    <li id="pos-discount-row" class="d-flex align-items-center justify-content-between gap-4 subtotal d-none">
        <span class="fw-semibold  py-4 fs-14 nowrap">
            {{ translate("Discount on POS") }}:
        </span>
        <span class="fw-semibold text-end pe-4 py-3 fs-14 nowrap" data-discount-on-pos="{{ 0 }}" id="pos-discount-amount">
            {{ show_amount(0) }}
        </span>
    </li>
    ...
    <li class="d-flex align-items-center justify-content-between gap-4 subtotal">
        ...
        <span class="fw-semibold text-end pe-4 py-3  fs-14 nowrap" data-shipping-fee="{{ $shippingCharge }}" id="shipping-fee">{{show_amount($shippingCharge)}}</span>
    </li>
    ...
    <li class="order-cost-item order-shipping-cost d-none d-flex align-items-center justify-content-between gap-4">
        <span class=" py-3 nowrap fs-14">{{translate("Shipping Charge")}} :</span>
    ...
    <li class="table-active d-flex align-items-center justify-content-between gap-4" data-sub="{{$total}}"
        id="subtotalamount">
        <h6 class=" py-3 nowrap fs-14 fw-bold">{{translate("Total")}} :</h6>
        <span class="text-end pe-4 py-3 nowrap fs-14">
    ...
    <li id="pos-payment-charge-row" class="d-flex align-items-center justify-content-between gap-4 d-none">
        <span class="fw-semibold  py-4 fs-14 nowrap">
            {{ translate("Payment Charge") }}:
        </span>
        <span class="fw-semibold text-end pe-4 py-3 fs-14 nowrap" data-payment-charge="{{ 0 }}" id="pos-payment-charge-amount">
            {{ show_amount(0) }}
        </span>
    </li>

    <li id="pos-payable-row" class="table-active  d-flex align-items-center justify-content-between gap-4 d-none">
        <span class="fw-bold  py-4 fs-14 nowrap">
            {{ translate("Payable") }}:
        </span>
        <span class="fw-bold text-end pe-4 py-3 fs-14 nowrap" data-payable="{{ 0 }}" id="pos-payable-amount">
            {{ show_amount(0) }}
        </span>
    </li>
    ...
    <li class="table-active d-flex align-items-center justify-content-between gap-4">
        <h6 class="py-3 nowrap fs-14 fw-bold">{{translate("Total Point Earned")}} :</h6>
        <span class="text-end pe-4 py-3 nowrap fs-14">
            <span id="totalamount" class="fw-bold">
