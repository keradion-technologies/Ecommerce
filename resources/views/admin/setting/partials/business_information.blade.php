            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-xl-6">
                    <label for="sku_prefix" class="form-label">
                        {{translate('SKU Prefix')}} <span class="text-danger" >*</span>
                    </label>
                    <input id="sku_prefix" type="text" name="site_settings[sku_prefix]" class="form-control" value="{{site_settings('sku_prefix')}}" placeholder="{{translate('SKU Prefix')}}">
                </div>

                <div class="col-xl-6">
                    <div>
                        <label for="pos_system" class="form-label">
                            {{translate('POS System')}}
                            <span class="text-danger" >* </span>
                            <i class="cursor-pointer ri-information-line"  data-bs-toggle="tooltip" data-bs-placement="top" title="{{translate('If you disable the POS system option, the POS Module will also be disabled.')}}"></i>
                        </label>

                        <select name="site_settings[pos_system]" id="pos_system"  class="form-select">
                            <option {{site_settings('pos_system',App\Enums\StatusEnum::true->status()) ==  App\Enums\StatusEnum::true->status() ? 'selected' : '' }} value="{{App\Enums\StatusEnum::true->status()}}">
                              {{translate('Active')}}
                           </option>
                            <option {{site_settings('pos_system',App\Enums\StatusEnum::true->status()) ==  App\Enums\StatusEnum::false->status() ? 'selected' : '' }} value="{{App\Enums\StatusEnum::false->status()}}">
                               {{translate('Inactive')}}
                           </option>
                       </select>
                    </div>
