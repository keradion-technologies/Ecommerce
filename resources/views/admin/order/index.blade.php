...
@section('main_content')
    <div class="page-content">
        <div class="container-fluid">
            ...
            @php
                $isPOS = request()->routeIs('admin.pos.*');
                $routePrefix = $isPOS ? 'admin.pos.system.inhouse.order.' : 'admin.inhouse.order.';
            @endphp
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">
                            {{ translate($isPOS ? 'Inhouse Pos Orders' : 'Inhouse Orders') }}
                        </li>
                    </ol>
                </div>
            </div>
            ...
            <div class="card" id="orderList">
                    ...
                    <div class="row g-4 align-items-center">
                        <div class="col-sm">
                            <h5 class="card-title mb-0">
                              {{ translate($isPOS ? 'Pos Order List' : 'Order List') }}
                            </h5>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="col-xl-4 col-sm-6">
                                <div class="search-box">
                                    <input type="text" id="datePicker" name="date" value="{{ request()->input('date') }}"
                                        class="form-control search" placeholder="{{ translate('Search by date') }}">
                ...
                <div class="card-body">
                    <ul class="nav nav-tabs nav-tabs-custom nav-primary mb-3" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'index') ? 'active' : '' }} py-3'
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'pending') ? 'active' : '' }} py-3'
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'placed') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'placed') }}">
                                <i class="ri-map-pin-line me-1 align-bottom"></i>
                                {{ translate('Placed Orders') }}
                                @if ($physical_product_order_count > 0 && !$isPOS )
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'confirmed') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'confirmed') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'processing') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'processing') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'shipped') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'shipped') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'delivered') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'delivered') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'cancel') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'cancel') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs($routePrefix . 'return') ? 'active' : '' }} py-3'
                                href="{{ route($routePrefix . 'return') }}">
                                ...
                                @forelse($orders as $order)
                                    <tr>
                                        <td data-label="{{ translate('Order Number') }}">
                                            <a href="{{ route('admin.inhouse.order.details', $order->id) }}"
                                                class="text-primary fw-bold" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="Order Number">
                                                {{ $order->order_id }}
                                            </a>
                                        </td>
                                        ...
                                        <td data-label="{{ translate('Source') }}">
                                            ...
                                            @if ($order->source == App\Enums\Pos\OrderSourceEnum::WEB->value)
                                                <span class="badge badge-soft-danger">{{ translate('Web') }}</span>
                                            @elseif ($order->source == App\Enums\Pos\OrderSourceEnum::POS->value)
                                                <span class="badge badge-soft-danger">{{ translate('Pos') }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td data-label="{{ translate('Action') }}">
                                            <div class="hstack justify-content-center gap-3">
                                                @if (permission_check('view_order'))
                                                    <a title="{{ translate('Print') }}"
                                                         href="{{ route($routePrefix . 'print', [$order->id, 'inhouse']) }}"
                                                        class="fs-18 link-success">
                                                        <i class="ri-printer-line"></i>
                                                    </a>
                                                    <a title="{{ translate('Details') }}" data-bs-toggle="tooltip"
                                                        data-bs-placement="top"
                                                        href="{{ route($routePrefix . 'details', $order->id) }}"
                                                        class="fs-18 link-info ms-1"><i class="ri-list-check"></i></a>
                                                @endif
                                                @if (permission_check('update_order'))
                                                    <a title="{{ translate('Order Status') }}" data-order_id="{{ $order->id }}"
                                                ...
                                                @if (permission_check('delete_order'))
                                                    @if ($order->status == App\Models\Order::CANCEL)
                                                        <a href="javascript:void(0);"
                                                            data-href="{{ route('admin.inhouse.order.delete', $order->id) }}"
                    ...
                    $('.status_html').html(`
                            <div class="form-group mt-2">
                                ...
                                <select class="form-select" name="status" id="status">
                                    ...
                                    <option value="1">{{ translate('Placed') }}</option>
                                    ...
                                    <option value="7">{{ translate('Return') }}</option>
                                </select>
                            </div>
                            <div class="form-group mt-2">
