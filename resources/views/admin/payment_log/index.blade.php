        ...
        <div class="container-fluid">
            ...
            <div class="card" id="orderList">
                <div class="card-header border-0">
                    <div class="row g-4">
                        <div class="col-lg-4 d-flex align-items-center">
                            @if(request()->routeIs('*.pos.system.*'))
                                <h5 class="card-title mb-0 flex-grow-1">
                                    {{translate('Pos Payment log list')}}
                                </h5>
                            @else
                                <h5 class="card-title mb-0 flex-grow-1">
                        ...
                        <div class="col-lg-8 d-flex align-items-end justify-content-end">
                            <div class="d-flex flex-wrap  align-items-end justify-content-end  gap-2">
                                ...
                                <a href="{{route('admin.payment.log.export')}}"
                                    class="btn btn-info btn-sm  waves ripple-light"> <i
                ...
                <div class="card-body pt-0">
                    @php
                        $isPos = request()->routeIs('admin.pos.system.payment.*');
                        $routePrefix = $isPos ? 'admin.pos.system.payment' : 'admin.payment';
                    @endphp
                    ...
                    <ul class="nav nav-tabs nav-tabs-custom nav-primary mb-3" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs($routePrefix . '.approved') ? 'active' : '' }} py-3"
                                href="{{ route($routePrefix . '.approved') }}">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs($routePrefix . '.rejected') ? 'active' : '' }} py-3"
                                href="{{ route($routePrefix . '.rejected') }}">
                                ...
                                @forelse($paymentLogs as $paymentLog)
                                    <tr>
                                        ...
                                        <td data-label="{{translate('user')}}">
                                            @if($paymentLog->user)
                                                <a href="{{route('admin.customer.details', $paymentLog->user_id)}}"
                                                    class="fw-bold text-dark">{{(@$paymentLog->user->name)}}</a><br>
                                        ...
                                        <td data-label="{{translate('Amount')}}">
                                            {{num_format(($paymentLog->amount))}} {{default_currency()->name}}
                                        </td>

                                        <td data-label="{{translate('Final Amount')}}">
                                            {{num_format(($paymentLog->final_amount))}}
                                            {{@$paymentLog->paymentGateway->currency->name ?? default_currency()->name}}
                                        </td>
