        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <ul class="nav nav-tabs nav-tabs-custom nav-primary mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{request()->routeIs('seller.transaction.history') ? 'active' :''}}   py-3"  id="Placed"
                            href="{{route('seller.transaction.history')}}" >
                            <i class="ri-user-line me-1 align-bottom"></i>
                            {{translate('Web Transactions')}}

                        </a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link {{request()->routeIs('seller.transaction.history.pos') ? 'active' :''}}   py-3"  id="Placed"
                            href="{{route('seller.transaction.history.pos')}}" >
                            <i class="ri-user-line me-1 align-bottom"></i>
                            {{translate('Pos Transactions')}}
                    ...
                    <table class="table table-hover table-nowrap align-middle mb-0" id="orderTable">
                        <thead class="text-muted table-light">
                            <tr class="text-uppercase">
                                ...
                                @if(request()->routeIs('seller.transaction.hisotry'))
                                    <th >{{translate('Post Balance')}}</th>
                                @elseif(request()->routeIs('seller.transaction.history.pos'))
                                ...
                                <tr>
                                    ...
                                    @if(request()->routeIs('seller.transaction.hisotry'))
                                        <td data-label="{{translate('Post Balance')}}">
                                            <span class="fw-bold">{{ short_amount($transaction->post_balance) }}</span>
                                        </td>
                                    @elseif(request()->routeIs('seller.transaction.history.pos'))
                                        <td data-label="{{translate('Post Pos Balance')}}">
                                            <span class="fw-bold">{{ short_amount($transaction->post_balance_pos) }}</span>
                                        </td>
                                    @endif
