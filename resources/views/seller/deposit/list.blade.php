						   ...
						   @forelse($reports as $report)
								<tr>
									...
									<td data-label="{{translate('Receivable')}}">
										{{num_format(($report->amount))}}{{default_currency()->name}}
									</td>

									<td data-label="{{translate('Charge')}}">
										<span class="text-danger fw-bold">	{{num_format(($report->charge),2)}} {{default_currency()->name}} </span>
									</td>

									<td data-label="{{translate('Paid')}}">
										<span class="fw-bold text-success">
											{{num_format(($report->final_amount))}} {{@$report->paymentGateway->currency->name ?? default_currency()->name}}										</span>
									</td>
