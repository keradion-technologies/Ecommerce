...
class ApiJsonResponse
{
    ...
    public function fails(string $message = '', int $httpCode = Response::HTTP_BAD_REQUEST, ?int $code = null): JsonResponse
    {
        ...
        try {
          ...
          if(request()->is('api/pos/admin/*')){
            $admin = auth()->guard('admin:api')->user() ;
            $response ['is_pos_admin_authenticate'] =  $admin && $admin->status == StatusEnum::true->status()
                                                                    ? true 
                                                                    : false;
          }

          if(request()->is('api/pos/seller/*')){
            $seller = auth()->guard('seller:api')->user() ;
            $response ['is_pos_seller_authenticate'] =  $seller && $seller->status == StatusEnum::true->status()
                                                                    ? true 
                                                                    : false;
          }
