...
class PaymentLog extends Model
{
    ...
    {
        return $query->whereHas('paymentGateway', function ($q) use ($sellerId) {
            $q->where('owner_type', 'seller')
    ...
    public function scopeFilter($q)
    {
        return $q->when(request()->input('trx_code'), function ($q) {
            return $q->where('trx_number', request()->input('trx_code'));
        })->when(request()->input('user_id'), function ($q) {
