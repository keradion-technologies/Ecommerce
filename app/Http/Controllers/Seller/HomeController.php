...
class HomeController extends Controller
{
    ...
    public function transaction()
    {
        ...
        $seller = Auth::guard('seller')->user();
        $transactions = Transaction::search()->web()->date()->whereNotNull('seller_id')->where('seller_id', $seller->id)->latest()->paginate(site_settings('pagination_number',10));
        return view('seller.transaction', compact('title', 'transactions'));
    }

    public function posTransaction()
    {
        $title = translate('Pos Transaction log');
        $seller = Auth::guard('seller')->user();
        $transactions = Transaction::search()->pos()->date()->whereNotNull('seller_id')->where('seller_id', $seller->id)->latest()->paginate(site_settings('pagination_number',10));
        return view('seller.transaction', compact('title', 'transactions'));
    }
