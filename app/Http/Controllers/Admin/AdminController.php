...
class AdminController extends Controller
{
     ...
     * @return View
     */
    public function index() :View | RedirectResponse | \Symfony\Component\HttpFoundation\StreamedResponse {
        ...
        $data['order_payment']             = num_format(Order::where('payment_status', Order::PAID)->sum('amount'));
        $data['total_withdraw']            = num_format(Withdraw::where('status', PaymentLog::SUCCESS)->sum('amount'));



        $data['total_customer']            = User::count();
        $data['total_seller']              = Seller::count();
        $data['total_payment']             = num_format(Order::where('payment_status', Order::PAID)->sum('amount')) +  
        num_format(PaymentLog::where('status', PaymentLog::SUCCESS)->sum('charge'))+
        num_format(Withdraw::where('status', PaymentLog::SUCCESS)->sum('charge'))
                                             +      $totalAmount;
