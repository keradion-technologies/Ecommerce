...
class InhouseProductOrderController extends Controller
{
    ...
    private function getFilteredOrdersWithTitle(?string $status = null)
    {
        $isPOS = request()->routeIs('admin.pos.*');
        $typeLabel = $isPOS ? 'POS' : 'Inhouse';
        ...
        $title = translate("$typeLabel " . ($statusMap[$status] ?? 'orders'));

        $query = Order::inhouseOrder()
            ->search()
            ->date()
            ->physicalOrder();

        if ($isPOS) {
            $query->adminOrder()->where('source', OrderSourceEnum::POS->value);
        }else{
            $query->where('source', OrderSourceEnum::WEB->value);
        }
        ...
        $orders = $query->orderBy('id', 'DESC')
            ->with('customer', 'shipping', 'shipping.method', 'orderDetails', 'orderDetails.product', 'paymentMethod')
            ->paginate(site_settings('pagination_number', 10))
            ->appends(request()->all());

        return compact('title', 'orders');
    }


    public function index()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle());
    }

    public function placed()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('placed'));
    }

    public function confirmed()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('confirmed'));
    }

    public function processing()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('processing'));
    }

    public function shipped()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('shipped'));
    }

    public function delivered()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('delivered'));
    }

    public function cancel()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('cancel'));
    }

    public function return()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('return'));
    }

    public function pending()
    {
        return view('admin.order.index', $this->getFilteredOrdersWithTitle('pending'));
    }
    ...
    public function details($id)
    {
        $isPOS = request()->routeIs('admin.pos.*');
        $typeLabel = $isPOS ? 'POS' : 'Inhouse';
        $title = translate("$typeLabel order details");
        ...
        $orderDeatils = OrderDetails::where('order_id', $order->id)
            ->inhouseOrderProduct()
            ->with('product')
            ->get();
    ...
    public function orderDetailStatusUpdate(Request $request)
    {
        ...
        if ($orderDeatils->status == Order::DELIVERED)
            return back()->with('error', translate('This order has already been delivered'));
        if ($orderDeatils->status == Order::RETURN)
            return back()->with('error', translate('This order has already been Returned'));
        ...
        if ($orderDeatils->product && $orderDeatils->product->seller) {
            ...
            $commission = 0;
            if (site_settings('seller_commission_status') == StatusEnum::true->status())
                $commission = (($orderDeatils->total_price * site_settings('seller_commission')) / 100);
