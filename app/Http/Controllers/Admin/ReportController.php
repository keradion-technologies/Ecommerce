...
class ReportController extends Controller
{
    ...
    public function sellerTransaction() :View
    {
        ...
        $transactions = Transaction::sellers()->web()->search()->date()->latest()->with('seller')->paginate(site_settings('pagination_number',10));
        return view('admin.report.index', compact('title', 'transactions'));
    }

    public function sellerTransactionPos() :View
    {
        $title        = translate('Seller Pos transactions');
        $transactions = Transaction::sellers()->pos()->search()->date()->latest()->with('seller')->paginate(site_settings('pagination_number',10));
        return view('admin.report.index', compact('title', 'transactions'));
    }
    ...
    public function userTransactionCsv(): \Symfony\Component\HttpFoundation\StreamedResponse{

        $fileHeaders = ['DATE','CUSTOMER','TRANSACTION_ID','AMOUNT', 'DETAILS'];
        $pluckableKeys = ['created_at', ['user' , 'email'], 'transaction_number' ,'amount' , 'details' ];

        return $this->csvExport(model: 'Transaction',fileNamePrefix:'deposit_log',fileHeader: $fileHeaders , pluckableKeys: $pluckableKeys);
    }
