...
class DeliveryManController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Add conditional rules for latitude and longitude based on Google Map status
        if ($is_map_enable) {
        ...
        $deliveryman->address = [
                'latitude'  => $is_map_enable ? $request->latitude : null,
                'longitude' => $is_map_enable ? $request->longitude : null,
    ...
    public function overview($id)
    {
        ...
        $rating = num_format(DeliveryManRating::with(['user'])
                        ->latest()
        ...
        $overview = [
            'withdraw' => [
                ...
                "total_success_withdraw"   => num_format(Withdraw::where('status', PaymentLog::SUCCESS)
                                                            ->whereNull('seller_id')
                                                            ->where('deliveryman_id',$id)->sum('amount')),


                "total_pending_withdraw"   => num_format(Withdraw::pending()
                                                        ->whereNull('seller_id')
                                                        ->where('deliveryman_id',$id)->sum('amount')),


                "total_rejected_withdraw"   => num_format(Withdraw::rejected()
                                                        ->whereNull('seller_id')
                                                        ->where('deliveryman_id',$id)->sum('amount'))
            ],
