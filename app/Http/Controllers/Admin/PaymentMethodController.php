...
class PaymentMethodController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $paymentMethods = PaymentMethod::search()
            ->with('currency')
            ->when(
                request()->routeIs('admin.gateway.payment.manual.*'),
                fn($q) => $q->manual(),
                fn($q) => request()->routeIs('admin.gateway.payment.pos.*') ? $q->forAdmin()->pos() : $q->automatic()
            )
            ->latest()
            ->get();
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $paymentMethod->type = $request->routeIs('admin.gateway.payment.pos.method.store') ? PaymentMethod::POS : PaymentMethod::MANUAL;
    ...
    public function update(Request $request, int $id): RedirectResponse
    {
        ...
        $this->validate($request, [
            'name' => ['nullable',Rule::requiredIf(fn() => in_array($paymentMethod->type, [PaymentMethod::MANUAL, PaymentMethod::POS])),'unique:payment_methods,name,' . $paymentMethod->id],
