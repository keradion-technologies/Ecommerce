...
class UserController extends Controller
{
    ...
    public function addressStore(Request $request) : JsonResponse{
        ...
        $is_map_enable = site_settings('gmap_status', 0) == StatusEnum::true->status();
        ...
        $address = [
                ...
                'latitude' => $is_map_enable ? $request->input('latitude') : null,
                'longitude' => $is_map_enable ? $request->input('longitude') : null
        ];
    ...
    public function addtocart(Request $request) :JsonResponse {
        ...
        if ($cart) {
            if($campaign){
                ...
                $cart->quantity = $quantity;
                $cart->total    =  (num_format($cart->price*$quantity));
                $cart->save();
            }
    ...
    public function updateCart(Request $request): JsonResponse
    {
        ...
        $stock =  ProductStock::where('product_id',$cartItem->product_id)
                                     ->where("attribute_value",$cartItem->attributes_value)
                                     ->first();

        if(!$stock || $request->quantity > $stock->qty) return api(['errors'=> ['Stock Not Available']])->fails(__('response.fail'));
    ...
    public function orderCheckout(Request $request): JsonResponse {
        ...
        if(!$user){
            ...
            $address = [
                'address' => $is_map_enable ? $request->input('address') : null,
                'latitude'=> $is_map_enable ? $request->input('latitude') : null,
        ...
        if($items->count() != 0){
            ...
            if($request->input('coupon_code')){
                ...
                if($coupon->seller && $request->input("product_ids")) {
                    ...
                    if($invalidProductExists) return api(['errors' => [translate('Sorry, your order contains ineligible products')]])->fails(__('response.fail')); 
                }

                $discount     = num_format(($coupon->discount($calculations['total_cart_amount'])));
    ...
    public function reedemPoint(Request $request) :JsonResponse {
        ...
        $amount = num_format($point / $conversionRate,4);
