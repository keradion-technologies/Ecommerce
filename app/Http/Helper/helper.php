...
if (!function_exists('access_check')) {
    function access_check($user, $module): bool
    {
        if (empty($user)) {
            return false;
        }
...
if (!function_exists(function: 'num_format')) {
    function num_format(mixed $amount, $decimalDigit = null)
    {

        $decimalDigit = $decimalDigit ?? site_settings('digit_after_decimal', 2);


        $format = '%.' . $decimalDigit . 'f';
        $formatted = sprintf($format, $amount);
...
if (!function_exists('count_product')) {
    function count_product($brand)
    {
        $brand = Brand::with([
            ...
            'product.review'
        ])->where('status', '1')->where('id', $brand->id)->firstOrFail();
...
if (!function_exists('limit_text')) {
    function limit_text(string $text, int $maxlimit = 10, string $endsWith = '....'): string
    {
        return strlen($text) > $maxlimit
            ? substr($text, 0, $maxlimit) . $endsWith
            : $text;
...
if (!function_exists('cal_discount_amount')) {
    function cal_discount_amount($discount, $price)
    {
...
if (!function_exists('getTaxes')) {
    function getTaxes(mixed $product, $stockPrice, $summation = true): int|float|\Illuminate\Support\Collection
    {
        $collection = (@$product->taxes->map(function (\App\Models\Tax $tax) use ($stockPrice): int|float{
            $pivot = @$tax->pivot;
            if ($pivot->type == 0)
                return ((($stockPrice / 100) * $pivot->amount));
...
if (!function_exists('getTaxesCollection')) {
    function getTaxesCollection(Product $product, $stockPrice, $summation = true): \Illuminate\Support\Collection
    {
        ...
        return (@$product->taxes->map(function (\App\Models\Tax $tax) use ($stockPrice): array{
            $pivot = @$tax->pivot;
            if ($pivot->type == 0)
                $price = ((($stockPrice / 100) * $pivot->amount));
...
if (!function_exists('generate_sku')) {
    function generate_sku(int $length = 8): string
    {
        $prefix = site_settings('sku_prefix') ?? 'SKU';
